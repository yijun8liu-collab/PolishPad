...
4. 继续输入时用输入框旁的 **追加/修改** 胶囊（`Tab` 键切换）选择语义：**追加（默认）**把你输入的当作新内容，润色后智能并入全文合适位置、已有内容不动；**修改**则是对当前版本提意见（"第二点缩短"）。每轮 `Enter` 后新版本自动撤销上一次粘贴再贴入，**原地替换**；满意 → 留空按 `Enter`、`Esc` 或 ✕ 结束，焦点留在原应用
