...
```json
{
  ...
  "hotkeyPolishSelection": "ctrl+option+r",  // 划词润色替换
  "hotkeyPolishAll": "ctrl+option+a",        // 全选润色替换
...
4. 继续输入时用输入框旁的 **追加/修改** 胶囊（`Tab` 键切换）选择语义：**追加（默认）**把你输入的当作新内容，润色后智能并入全文合适位置、已有内容不动；**修改**则是对当前版本提意见（"第二点缩短"）。每轮 `Enter` 后新版本自动删除上一版内容再贴入，**原地替换**；满意 → 留空按 `Enter`、`Esc` 或 ✕ 结束，焦点留在原应用
5. 右上角 **中/EN 开关**：EN 模式下润色结果输出英文（使用原生英文提示词），界面文字同步切换；选择会被记住，划词模式也跟随
...
- **菜单栏入口**：点 ✨ 图标 → 「润色选中文本」/「全选润色替换」（后者不需要先选中任何文字）

处理期间鼠标旁会出现「润色中…」气泡、菜单栏图标变为沙漏，完成后气泡变绿色 ✓ 并播放提示音；失败时弹窗说明，**原文不会被修改**。抓取/回贴临时借用剪贴板，完成后会自动恢复你原来的剪贴板内容（包括图片等非文本）。
...
已覆盖：多轮纠偏与原地替换、追加/修改双模式（Tab 切换）、自动粘贴回原应用、划词/全选润色（快捷键、右键服务、菜单栏三种入口）、语音输入（macOS 原生识别，长停顿不丢内容）、中/EN 双语（输出语言 + 界面语言 + 独立双语提示词）、应用内设置界面（快捷键按键录制、保存即生效）、HUD 过程反馈、IME 回车保护、请求取消、失败保留原文、剪贴板快照完整恢复、疑似截断提醒、快捷键冲突提示、多显示器、全屏 Space 可用。
