...
已覆盖：多轮纠偏与原地替换、自动粘贴回原应用、划词/全选润色（快捷键、右键服务、菜单栏三种入口）、语音输入（macOS 原生识别，长停顿不丢内容）、中/EN 双语（输出语言 + 界面语言 + 独立双语提示词）、应用内设置界面、HUD 过程反馈、IME 回车保护、请求取消、失败保留原文、剪贴板快照完整恢复、疑似截断提醒、快捷键冲突提示、多显示器、全屏 Space 可用。
