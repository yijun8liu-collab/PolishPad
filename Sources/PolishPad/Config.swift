...
struct AppConfig: Codable {
    ...
    场景设定：这份草稿几乎总是发给 AI 编程助手的技术消息，主题围绕软件工程与机器学习。因此：
    - 歧义词一律优先按编程/ML 语境理解（如残差、切片、收益、部署等）；
    - 代码标识符、文件名、命令、API/库/模型名保持原样或还原为正确的英文写法，不要翻译成中文；
    - 把诉求整理成 AI 助手好执行的形式：任务边界清晰、指代明确、多个诉求分点列出；涉及报错或异常现象时，完整保留错误信息、复现条件等关键细节，不要概括掉。
